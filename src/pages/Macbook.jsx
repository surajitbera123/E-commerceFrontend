...
import "../App.css"
// import { macMainPic, macbookData } from '../datas/provider/macbook'
import Items from '../components/Items'
import Footer from "../components/Footer"

const Macbook = () => {  
  const[macbookData,setmacbookData]=useState("");
  const[macbookMainPic,setmacbookMainPic]=useState("");

  useEffect(()=>{
    fetch("https://ecommerce-backend-git-main-surajitbera123.vercel.app/macbookData")
    .then(res=>res.json())
    .then(json=>setmacbookData(json))
    .catch(err=>console.log("error"))
  },[])
  useEffect(()=>{
    fetch("https://e-commerce-backend1-96te426ns-surajitbera123.vercel.app/macbookMainPic")
    .then(res=>res.json())
    .then(json=>setmacbookMainPic(json[0]))
    .catch(err=>console.log("error"))
  },[])
  ...
  return (
    ...
    <span style={{display:"inline-block",width:"55%"}} className="rightAbsolute">
      <img key={macbookMainPic.id} className='rightBoxOfPages' src={macbookMainPic.img} alt=''/><br/><br/>
      ...
      {
          macbookData && macbookData.map((val)=>{
            return(
              <>
