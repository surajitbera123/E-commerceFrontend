...
import "../App.css"
// import { ipadMainPic, ipadData } from '../datas/provider/ipad'
...
const Ipad = () => {
  ...
  const[ipadMainPic,setipadMainPic]=useState("");

  useEffect(()=>{
    fetch("https://ecommerce-backend-git-main-surajitbera123.vercel.app/ipadData")
    .then(res=>res.json())
    .then((json)=>{setipadData(json)})
    .catch(err=>console.log("error"))
  },[])
  useEffect(()=>{
    fetch("https://ecommerce-backend-git-main-surajitbera123.vercel.app/ipadMainPic")
    .then(res=>res.json())
    .then(json=>setipadMainPic(json[0]))
    .catch(err=>console.log("error"))
  },[])
  ...
  return (
    ...
    <span style={{display:"inline-block",width:"55%"}} className="rightAbsolute">
      <img key={ipadMainPic.id} className='rightBoxOfPages' src={ipadMainPic.img} alt=''/><br/><br/>
